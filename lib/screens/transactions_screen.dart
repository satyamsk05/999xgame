...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  List<TransactionItemData> _serverTransactions = [];
  bool _loading = true;

  @override
  void initState() { super.initState(); _selectedFilter = widget.initialFilter; _loadHistory(); }

  Future<void> _loadHistory() async {
    try {
      final res = await WalletApi.getTransactions(page: 1, limit: 50);
      final items = res['items'] is List ? res['items'] as List : [];
      final parsed = <TransactionItemData>[];
      for (final raw in items) {
        if (raw is! Map) continue;
        parsed.add(TransactionItemData(
          id: raw['id']?.toString() ?? '',
          title: raw['title']?.toString() ?? 'Transaction',
          ...
          utr: raw['utr']?.toString(),
        ));
      }
      if (mounted) setState(() { _serverTransactions = parsed; _loading = false; });
    } catch (_) {
      if (mounted) setState(() { _serverTransactions = widget.transactions; _loading = false; });
    }
  }

  List<TransactionItemData> get _filteredTransactions {
    final source = _serverTransactions.isNotEmpty || _loading ? _serverTransactions : widget.transactions;
    if (_selectedFilter == 'Deposit') return source.where((t) => t.category == 'Deposit').toList();
    if (_selectedFilter == 'Withdraw') return source.where((t) => t.category == 'Withdrawal' || t.category == 'Withdraw').toList();
    return source;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(color: const Color(0xFF1B0326), child: SafeArea(child: Column(children: [
      Padding(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12), child: Row(children: [IconButton(icon: const Icon(Icons.arrow_back, color: Colors.white, size: 24), onPressed: widget.onBackPressed), const SizedBox(width: 12), Text('Transaction History', style: GoogleFonts.poppins(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w700))])),
      ...
      Expanded(child: _loading ? const Center(child: CircularProgressIndicator()) : items.isEmpty ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.receipt_long_outlined, size: 56, color: Colors.white30), const SizedBox(height: 12), Text('No transactions found for $_selectedFilter', style: GoogleFonts.poppins(color: Colors.white54, fontSize: 14))])) : ListView(padding: const EdgeInsets.symmetric(horizontal: 16), children: [Text('RECENT ACTIVITY', style: GoogleFonts.poppins(color: Colors.white54, fontSize: 12, fontWeight: FontWeight.w600, letterSpacing: 1)), const SizedBox(height: 12), ...items.map(_buildTransactionCard)])),
    ])));
  }

  Widget _buildFilterTab(String label) { final selected = _selectedFilter == label; return GestureDetector(onTap: () => setState(() => _selectedFilter = label), child: AnimatedContainer(duration: const Duration(milliseconds: 180), padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 10), decoration: BoxDecoration(color: selected ? const Color(0xFF6B1884) : Colors.transparent, borderRadius: BorderRadius.circular(24), border: Border.all(color: selected ? const Color(0xFF6B1884) : Colors.white38, width: 1.2)), child: Text(label, style: GoogleFonts.poppins(color: Colors.white, fontSize: 14, fontWeight: selected ? FontWeight.w700 : FontWeight.w500)))); }

  Widget _buildTransactionCard(TransactionItemData item) {
    final isDeposit = item.category == 'Deposit';
    return Column(children: [Padding(padding: const EdgeInsets.symmetric(vertical: 13), child: Row(children: [
      ...
      const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.poppins(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w600)), const SizedBox(height: 2), Text(_formatTimestamp(item.timestamp), style: GoogleFonts.poppins(color: Colors.white54, fontSize: 11)), if (isDeposit && item.status != null) ...[const SizedBox(height: 5), _statusBadge(item.status!)] ])), const SizedBox(width: 8), Text(item.isCredit ? '+ ₹${item.amount.toStringAsFixed(item.amount % 1 == 0 ? 0 : 2)}' : '- ₹${item.amount.toStringAsFixed(item.amount % 1 == 0 ? 0 : 2)}', style: GoogleFonts.inter(color: item.isCredit ? const Color(0xFF00E676) : Colors.white, fontSize: 15, fontWeight: FontWeight.w800)),
