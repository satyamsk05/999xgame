...
class CrushEngine {
  ...
  /**
   * Serialize only fields that are safe for clients.
   * The committed crash point is NEVER exposed before CRASHED/SETTLED.
   * The server seed is only revealed after the round has crashed/settled.
   */
  ...
  getCurrentMultiplier() {
    if (!this.currentRound || this.currentRound.status !== RoundStatus.FLYING || !this.flyingStartedAt) return 1.0;
  ...
  async cashoutBet({ betId, userId }) {
    ...
    return await crushRepo.cashoutBetInDb({ betId, userId, roundId: this.currentRound.roundId, cashoutMultiplier: serverMultiplier });
  }

  async processAutoCashouts() {
    if (!this.currentRound || this.currentRound.status !== RoundStatus.FLYING) return [];
    return await crushRepo.processAutoCashoutsInDb(this.currentRound.roundId, this.getCurrentMultiplier());
  }
  ...
  async recoverFromDb() {
    ...
    if (rounds.length) logger.warn('Crush recovery processed orphaned rounds', { total: rounds.length, settled, refunded });
