...
const ENGINES = { seven_up_down: sevenUpDownEngine, dragon_tiger: dragonTigerEngine, crush: crushEngine };

async function requireLiveGame(gameId) {
  const canonicalId = GAME_ALIASES[gameId] || gameId;
  if (!ENGINES[canonicalId]) { const error = new Error(`Unknown game: ${gameId}`); error.statusCode = 404; throw error; }
  const result = await query('SELECT status FROM games WHERE id = $1 LIMIT 1', [canonicalId]);
  if (result.rows[0]?.status !== 'LIVE') { const error = new Error('Game is currently unavailable.'); error.statusCode = 409; error.code = 'GAME_NOT_LIVE'; throw error; }
...
router.post('/join', betLimiter, authMiddleware, async (req, res) => {
  try {
    ...
    const requestKey = validation.validateIdempotencyKey(idempotencyKey) || `sud_req_${crypto.randomUUID()}`;
    const { bet, isDuplicate } = await sevenUpDownEngine.placeBet({ userId: req.user.id, betType, stakePaise: computedStakePaise, idempotencyKey: requestKey });
