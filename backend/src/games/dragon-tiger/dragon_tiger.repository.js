  ...
  try {
    const res = await query(
      `UPDATE game_rounds
       ...
       WHERE id = $1
       RETURNING *`,
      [roundId, status, dragonCard ? JSON.stringify({ dragonCard, tigerCard, winningBetType }) : null]
    );
    return res.rows[0];
  } catch (err) {
...
async function placeBetInDb({ userId, roundId, betType, stakePaise, idempotencyKey }) {
  ...
  if (!Number.isInteger(stake) || stake <= 0) {
    const e = new Error('Stake must be a positive integer in paise'); e.statusCode = 400; throw e;
  }
  const limits = await getStakeLimits('dragon_tiger', DEFAULT_STAKE_LIMITS_PAISE);
  if (stake < limits.min) { const e = new Error(`Minimum bet is ₹${(limits.min / 100).toFixed(2)}`); e.statusCode = 400; throw e; }
  if (stake > limits.max) { const e = new Error(`Maximum bet is ₹${(limits.max / 100).toFixed(2)}`); e.statusCode = 400; throw e; }
  ...
  try {
    ...
    if (!roundRes.rows.length || roundRes.rows[0].status !== 'BETTING_OPEN') {
      const e = new Error('Betting is closed for this round'); e.statusCode = 409; throw e;
    }

    if (idempotencyKey) {
      const dupRes = await client.query(
        'SELECT * FROM bets WHERE idempotency_key = $1 FOR UPDATE',
        [idempotencyKey]
      );
      if (dupRes.rows.length > 0) {
        const existing = dupRes.rows[0];
        if (String(existing.user_id) !== String(userId)) {
          const e = new Error('Idempotency key is already used by another account');
          e.statusCode = 409;
          e.code = 'IDEMPOTENCY_KEY_CONFLICT';
          throw e;
        }
    ...
    if (betInsertRes.rows.length === 0) {
      const existingRes = await client.query('SELECT * FROM bets WHERE idempotency_key = $1 FOR UPDATE', [idempotencyKey]);
      const existing = existingRes.rows[0];
      if (!existing) {
        const e = new Error('Unable to resolve idempotency conflict'); e.statusCode = 409; throw e;
      }
      if (String(existing.user_id) !== String(userId)) {
        const e = new Error('Idempotency key is already used by another account');
        e.statusCode = 409;
...
async function settleRoundInDb(roundId, winningBetType) {
  ...
  try {
    await client.query('BEGIN');
    const roundRes = await client.query('SELECT id, status FROM game_rounds WHERE id = $1 FOR UPDATE', [roundId]);
    if (!roundRes.rows.length) { const e = new Error(`Cannot settle: round ${roundId} not found`); e.statusCode = 404; throw e; }
    if (['SETTLED', 'CANCELLED'].includes(roundRes.rows[0].status)) { await client.query('COMMIT'); return { settlements: [], alreadySettled: true }; }
    ...
    for (const bet of betsRes.rows) {
      ...
      let winAmountPaise = 0; let newStatus = 'LOST';
      if (isWin) { winAmountPaise = Math.floor(parseInt(bet.stake, 10) * parseFloat(bet.payout_multiplier)); newStatus = 'WON'; }
      else if (isTiePush) { winAmountPaise = Math.floor(parseInt(bet.stake, 10) * DRAGON_TIGER_TIE_PUSH_RATIO); newStatus = 'REFUNDED'; }
      if (winAmountPaise > 0) await financialService.creditWallet(client, winAmountPaise, { userId: bet.user_id, type: isTiePush ? 'REFUND' : 'WIN_CREDIT', referenceType: isTiePush ? 'GAME_BET' : 'GAME_WIN', referenceId: bet.id, idempotencyKey: `dt_win_${bet.id}`, metadata: { gameId: 'dragon_tiger', roundId, betId: bet.id, winAmountPaise, tiePush: isTiePush } });
      await client.query(`UPDATE bets SET status = $2, win_amount = $3, settled_at = NOW() WHERE id = $1`, [bet.id, newStatus, winAmountPaise]);
      await client.query(`INSERT INTO settlements (id, bet_id, round_id, user_id, win_amount, status, metadata, created_at) VALUES ($1, $2, $3, $4, $5, 'SETTLED', $6, NOW()) ON CONFLICT (bet_id) DO NOTHING`, [`dt_stl_${bet.id}`, bet.id, roundId, bet.user_id, winAmountPaise, JSON.stringify({ winningBetType, tiePush: isTiePush })]);
  ...
  } catch (err) { await client.query('ROLLBACK'); logger.error('Failed to settle Dragon Tiger round in DB', { roundId, error: err.message }); throw err; }
  finally { client.release(); }
}
...
async function refundRoundInDb(roundId, reason = 'ROUND_VOIDED') {
  ...
  try {
    await client.query('BEGIN');
    const roundRes = await client.query('SELECT id, status FROM game_rounds WHERE id = $1 FOR UPDATE', [roundId]);
    if (!roundRes.rows.length) { await client.query('COMMIT'); return { refunded: 0 }; }
    if (['SETTLED', 'CANCELLED'].includes(roundRes.rows[0].status)) { await client.query('COMMIT'); return { refunded: 0, alreadyFinal: true }; }
    const betsRes = await client.query(`SELECT * FROM bets WHERE round_id = $1 AND status = 'ACCEPTED' FOR UPDATE`, [roundId]);
    for (const bet of betsRes.rows) {
      const stake = parseInt(bet.stake, 10);
      await financialService.creditWallet(client, stake, { userId: bet.user_id, type: 'REFUND', referenceType: 'GAME_BET', referenceId: bet.id, idempotencyKey: `dt_refund_${bet.id}`, metadata: { gameId: 'dragon_tiger', roundId, betId: bet.id, reason } });
      await client.query(`UPDATE bets SET status = 'REFUNDED', win_amount = 0, settled_at = NOW() WHERE id = $1`, [bet.id]);
      await client.query(`INSERT INTO settlements (id, bet_id, round_id, user_id, win_amount, status, metadata, created_at) VALUES ($1, $2, $3, $4, 0, 'REFUNDED', $5, NOW()) ON CONFLICT (bet_id) DO NOTHING`, [`dt_stl_${bet.id}`, bet.id, roundId, bet.user_id, JSON.stringify({ reason })]);
    }
  ...
  } catch (err) { await client.query('ROLLBACK'); logger.error('Failed to refund Dragon Tiger round', { roundId, error: err.message }); throw err; }
  finally { client.release(); }
}

async function getInFlightRoundsFromDb() {
  const res = await query(`SELECT id as "roundId", round_number as "roundNumber", status, result, server_seed as "serverSeed", server_seed_hash as "serverSeedHash", created_at as "createdAt" FROM game_rounds WHERE game_id = 'dragon_tiger' AND status IN ('CREATED','BETTING_OPEN','BETTING_CLOSED','RESULT','SETTLING') ORDER BY created_at ASC`);
  return res.rows.map(row => ({ roundId: row.roundId, roundNumber: row.roundNumber, status: row.status, dragonCard: row.result?.dragonCard ?? null, tigerCard: row.result?.tigerCard ?? null, winningBetType: row.result?.winningBetType ?? null, serverSeed: row.serverSeed ?? null, serverSeedHash: row.serverSeedHash ?? null, createdAt: row.createdAt }));
}

module.exports = { createRoundInDb, updateRoundInDb, getStakeLimits, getPayoutMultiplier, placeBetInDb, settleRoundInDb, refundRoundInDb, getInFlightRoundsFromDb };
