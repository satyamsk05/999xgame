  ...
  try {
    ...
    const countRes = await query(
      `SELECT COUNT(*)::int AS total FROM users
       WHERE ($1 = '' OR username ILIKE $2 OR phone ILIKE $2 OR id ILIKE $2)`,
      [search, like]
    );
    const rowsRes = await query(
      `SELECT u.id, u.username, u.phone, u.is_blocked, u.created_at, u.kyc_status,
              ...
              COALESCE(w.rewards_balance,0) AS rewards_balance
       FROM users u
       LEFT JOIN wallets w ON w.user_id = u.id
       WHERE ($1 = '' OR u.username ILIKE $2 OR u.phone ILIKE $2 OR u.id ILIKE $2)
    ...
    const data = rowsRes.rows.map((u) => {
      ...
      return {
        ...
        wallet: {
          ...
          reservedBalance: reserved / 100,
          depositBalance: parseInt(u.deposit_balance || 0, 10) / 100,
          winningsBalance: parseInt(u.winnings_balance || 0, 10) / 100,
          rewardsBalance: parseInt(u.rewards_balance || 0, 10) / 100,
          totalBalance: (available + reserved) / 100,
        },
      };
    });
...
// GET /api/admin/users/:userId — user profile, wallet and recent ledger activity.
router.get('/:userId', async (req, res, next) => {
  try {
    ...
    const txRes = await query(
      `SELECT id, type, amount, direction, created_at, reference_id
       FROM wallet_ledger WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50`,
      [userId]
    );
    const recentTransactions = txRes.rows.map((t) => ({
      id: t.id, type: t.type, amount: parseInt(t.amount || 0, 10) / 100,
      direction: t.direction, createdAt: t.created_at, referenceId: t.reference_id,
    }));
    res.status(200).json({ status: 'success', data: {
      user: {
        id: u.id, username: u.username, phone: u.phone, avatarPath: u.avatar_path,
        isBlocked: !!u.is_blocked, blockedAt: u.blocked_at, blockedReason: u.blocked_reason,
        kycStatus: u.kyc_status || 'NOT_SUBMITTED', createdAt: u.created_at,
      },
      wallet: {
        availableBalance: available / 100, reservedBalance: reserved / 100,
        depositBalance: parseInt(w.deposit_balance || 0, 10) / 100,
        winningsBalance: parseInt(w.winnings_balance || 0, 10) / 100,
        rewardsBalance: parseInt(w.rewards_balance || 0, 10) / 100,
        totalBalance: (available + reserved) / 100,
      },
      recentTransactions,
      recentBets: [],
