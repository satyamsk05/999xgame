...
router.post('/:userId/adjust-balance', requireRole('SUPER_ADMIN', 'FINANCE_ADMIN'), async (req, res, next) => {
  ...
  try {
    ...
    logger.info('Admin manual balance adjustment', { adminId, userId, direction, amountPaise, reason: reason.trim(), referenceId, idempotencyKey });
    res.status(200).json({
      status: 'success',
      message: `Wallet ${direction === 'CREDIT' ? 'credited' : 'debited'} ₹${amount} successfully.`,
      data: {
        referenceId, userId, direction, amountRupees: Number(amount),
        newBalance: { availableBalance: avail / 100, reservedBalance: resv / 100, totalBalance: (avail + resv) / 100 },
      },
    });
  } catch (err) {
...
router.post('/:userId/block', requireRole('SUPER_ADMIN', 'SUPPORT_ADMIN'), async (req, res, next) => {
  try {
    ...
    await query(`INSERT INTO audit_logs (id, user_id, action, details, created_at) VALUES ($1, $2, 'USER_BLOCKED', $3::jsonb, NOW())`,
...
router.post('/:userId/unblock', requireRole('SUPER_ADMIN', 'SUPPORT_ADMIN'), async (req, res, next) => {
  try {
    ...
    await query(`INSERT INTO audit_logs (id, user_id, action, details, created_at) VALUES ($1, $2, 'USER_UNBLOCKED', $3::jsonb, NOW())`,
...
router.patch('/:userId/kyc', requireRole('SUPER_ADMIN', 'SUPPORT_ADMIN'), async (req, res, next) => {
  try {
    ...
    await query(`INSERT INTO audit_logs (id, user_id, action, details, created_at) VALUES ($1, $2, 'USER_KYC_UPDATE', $3::jsonb, NOW())`,
