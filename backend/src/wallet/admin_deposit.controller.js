 ...
 */
router.post('/:depositId/confirm', async (req, res, next) => {
  try {
...
/**
 * POST /api/admin/deposits/:depositId/reject — Reject deposit order (Zero wallet credit)
 */
router.post('/:depositId/reject', async (req, res, next) => {
  try {
    const { depositId } = req.params;
    const { adminNote } = req.body || {};
    const adminId = req.admin ? req.admin.id : 'admin_sys';
