...
async function getTransactionsByUserId(userId, category = 'All') {
  try {
    ...
    let sql = 'SELECT * FROM wallet_ledger WHERE user_id = $1';
    const params = [userId];
    if (normalizedCategory !== 'all') {
      const types = categoryTypes[normalizedCategory];
      if (!types) {
        const err = new Error('Invalid transaction category');
        err.statusCode = 400;
        throw err;
      }
    ...
    return res.rows.map((row) => ({ id: row.id, title: formatLedgerTitle(row.type, row.reference_id), amount: parseInt(row.amount, 10) / 100, isCredit: row.direction === 'CREDIT', timestamp: row.created_at, category: mapLedgerCategory(row.type) }));
